...
int lua_load_all(lua_State *L, struct syscall_function_t *f) {
    int ret = 0;
    lua_newtable(L);
    size_t index = 0;
    while (1) {
...
int lua_ckb_load_all_cell_data(lua_State *L) {
    FIELD fields[] = {
        {"source", SIZE_T},
    };
...
static const luaL_Reg ckb_syscall[] = {
    ...
    {"load_all_cell_data", lua_ckb_load_all_cell_data},
